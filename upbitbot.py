...
def load_upbit(name):
    if name == 'aws':
        access = "75echrCjA6hz1BWOUK0c4hhTTNLBTvXKxAY0Nj9z" # Access key
        secret = "V63H99gngKw1Uy4qKC9KxKWxKfse34EvXavAZA3N" # Secret Key
...
while True:
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=10):
            ...
            #print('ma20:',ma20)
            if (now_rsi < 27): # test1>0 and test2<0 (target_price < current_price)
                krw = get_balance('KRW')
                if krw > 5000:
                    upbit.buy_market_order('KRW-ETC',krw*0.9995)
        else:
            etc = get_balance('ETC')
            if (now_rsi > 65): # test1<0 and test2>0 (etc > 0.00008)
